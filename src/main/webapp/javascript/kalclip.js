...
clipApp.doSave = function() {
  ...
  $("#replace").prepend( clipApp.disableDiv() );
  ...
  var params = {
    ...
    'desc': $("#entry_desc").val(),
    'saveToSiteLibrary' : $("#addToSiteLibrary").is(':checked'),
    'locationId' : $("#locationId").val()
  };

  //expects a URL like: PUT /kaltura/{entryId}/saveClip/{start}/{end}
  var entryId = clipApp.vars.entry.id;
  var saveUrl = '/direct/kaltura/' + entryId + '/saveClip/' + 
                $("#startTime").timeStepper( 'getValue' ) + '/' + $("#endTime").timeStepper( 'getValue' ) + ".json";
  ...
  $.ajax({
    ...
    error: function( jqXHR, textStatus, errorThrown ) {
      alert("Failure trying to save entry ("+entryId+") as clip ("+saveUrl+"): "+textStatus+": status="+jqXHR.status+" "+jqXHR.statusText );
    }
  });
};

clipApp.doReplace = function() {
  if( ($("#endTime").timeStepper( 'getValue' ) - $("#startTime").timeStepper( 'getValue' )) <= 100 ) {
    alert('Clip duration must be bigger then 100ms.');
    return ;
  }
  ...
  var replaceUrl = '/direct/kaltura/' + entryId + '/replaceClip/' + 
                $("#startTime").timeStepper( 'getValue' ) + '/' + $("#endTime").timeStepper( 'getValue' ) +
                '/' + collectionId + '/' + $("#position").val() + ".json";
  ...
  $.ajax({
    ...
    error: function( jqXHR, textStatus, errorThrown ) {
      alert("Failure replacing entry ("+entryId+") with clip in collection ("+collectionId+"): "+textStatus+": status="+jqXHR.status+" "+jqXHR.statusText );
    }
  });
};
