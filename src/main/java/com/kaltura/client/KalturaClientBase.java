...
abstract public class KalturaClientBase {
    ...
    private static Log logger = LogFactory.getLog(KalturaClientBase.class);
    
    private Header[] lastRequestHeaders = null; 
	...
	protected String executeMethod(HttpClient client, PostMethod method) {
		...
		try {
			...
			for(Header header : headers)
			    if (logger.isDebugEnabled()) logger.debug("Header [" + header.getName() + " value [" + header.getValue() + "]");
            ...
            lastRequestHeaders = method.getResponseHeaders();
			...
			if(responseString.length() < MAX_DEBUG_RESPONSE_STRING_LENGTH) {
			    if (logger.isDebugEnabled()) logger.debug(responseString);
			} else {
			    if (logger.isDebugEnabled()) logger.debug("Received long response. (length : " + responseString.length() + ")");
			}
	...
	public String generateSession(String adminSecretForSigning, String userId, KalturaSessionType type, int partnerId, int expiry, String privileges) throws Exception
	{
		try
		{
			...
			// NOTE: do not use sun classes -AZ
			...
			// ks = ks.replace("\r", "");
			// NOTE: the other way does not get all linebreaks, only unix ones, not really sure why there are any line breaks to begin with though... -AZ
			String ks = hashedString.replaceAll("\\r\\n|\\r|\\n", "");
